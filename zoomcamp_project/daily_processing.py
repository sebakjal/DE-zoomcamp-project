...
# Main function to execute on daily data
def pyspark_process():

    credentials_locations = "/home/kjal/zoomcamp_project/credentials.json"  # path to GCP credentials
    ...
    # Reading data from 'yesterday' earthquakes
    daily_df = spark.read \
        .option('header', 'true') \
        .schema(schema) \
        .csv(f'gs://{gcs_daily_bucket}/{yesterday}_events.csv')
    ...
    for validation_name, validation_func in validation_rules:
        result = validation_func(daily_df)
        if result.count() > 0:
    ...
    # Creating new fields    
    df_with_percentages = daily_df.withColumn('depthError_percent', col('depthError')/col('depth')*100) \
                                .withColumn('magError_percent', col('magError')/col('mag')*100)

    # Loading the new data into Bigquery
...
gcs_daily_bucket = os.environ.get("GCS_DAILY_BUCKET")   
